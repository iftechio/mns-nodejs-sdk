...
test.serial('test subscription', async t => {
  ...
  await client.subscribe({
    TopicName: TEST_TOPIC_NAME, SubscriptionName: TEST_SUBSCRIBE_NAME, Attributes: {
      Endpoint: `${client.endpoint}/${TEST_QUEUE_NAME}`
    }
  })

  const subscriptions = await client.listSubscriptionByTopic({ TopicName: TEST_TOPIC_NAME })
  t.true(subscriptions.Subscription.map(subscription => subscription.SubscriptionName).includes(TEST_SUBSCRIBE_NAME))
  ...
  const attributes = await client.getSubscriptionAttributes({ TopicName: TEST_TOPIC_NAME, SubscriptionName: TEST_SUBSCRIBE_NAME })
...
test.serial('test publish', async t => {
  ...
  await client.publishMessage({ TopicName: TEST_TOPIC_NAME, Payloads: { MessageBody: TEST_MESSAGE_BODY } })
